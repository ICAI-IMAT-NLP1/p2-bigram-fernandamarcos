...
def bigrams_count_to_probabilities(
    ...
    probabilities = torch.nn.functional.normalize(smoothed, p=1 ,dim = 1)
...
def generate_name(
    ...
    while len(generated_name) < max_length:
        ...
        # Sample the next character based on the current char
        next_char = sample_next_character(current_char_index, bigram_probabilities, idx_to_char)

        # If the next character is the end token, stop generating
        if next_char == end_token:
            break

        # Add the next character to the generated name
        generated_name += next_char

        # Update the current char for the next iteration
